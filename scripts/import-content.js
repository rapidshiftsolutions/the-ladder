 ...
 * 
 * This script imports content scraped from the production site (https://www.the-ladder.org)
 * into the Sanity CMS.
 * 
 * Usage: SANITY_TOKEN=your_token node scripts/import-content.js
 * 
 * Required environment variable:
 *   SANITY_TOKEN - A Sanity API token with write permissions
 */

const { createClient } = require('@sanity/client')

// Sanity configuration
const projectId = process.env.NEXT_PUBLIC_SANITY_PROJECT_ID || '9a1830p7'
...
if (!token) {
  ...
  console.log('\nGet a token from: https://sanity.io/manage/project/' + projectId + '/api#tokens')
  process.exit(1)
}
...
async function importSuccessStories() {
...
async function importPartnerOrganizations() {
  console.log('\n🤝 Importing Partner Organizations...')
  ...
  for (const partner of partnerOrganizations) {
    ...
    try {
      ...
      console.error(`  ❌ ${partner.name}: ${error.message}`)
    }
  }
}

async function importProcessSteps() {
  ...
  for (const step of processSteps) {
    ...
    try {
      ...
      console.error(`  ❌ Step ${step.stepNumber}: ${error.message}`)
    }
  }
}
...
async function main() {
  ...
  try {
    await importSuccessStories()
    await importTeamMembers()
    await importPartnerOrganizations()
    await importProcessSteps()
